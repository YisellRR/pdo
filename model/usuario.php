...
class usuario
{
	...
	public function EliminarRoll($id)
	{
		try 
		{
			$stm = $this->pdo
			            ->prepare("DELETE FROM roles WHERE id_usuario = ? AND sucursal IS NOT NULL");			          
	...
	public function RegistrarRoll(usuario $data)
	{
		try 
		{
		$sql = "INSERT INTO roles (id_usuario, sucursal) 
		        VALUES (?, ?)";
