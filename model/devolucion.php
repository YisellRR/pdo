...
class devolucion
{
	...
	public function Listar($id_venta)
	{
		try
		{
			...
			if($id_venta==0){
				...
				v.id_producto,
				v.id_sucursal, 
				(SELECT sucursal FROM sucursales s WHERE s.id = v.id_sucursal) as sucursal,
	...
	public function ListarProducto($id_producto, $desde, $hasta, $sucursal)
	{
		try
		{
			$sucursal=$sucursal != '' ?"AND v.id_sucursal = '$sucursal'" : '';
			...
			FROM devoluciones v  
			LEFT JOIN productos p ON v.id_producto = p.id 
			LEFT JOIN clientes c ON v.id_cliente = c.id WHERE v.id_producto = ? $rango AND v.anulado = 0 $sucursal GROUP BY v.id_venta");
	...
	public function Registrar($data)
	{
		...
		$sql = "INSERT INTO devoluciones (id_venta, id_cliente, id_vendedor, vendedor_salon, id_producto, precio_costo, precio_venta, subtotal, descuento, iva, total, comprobante, nro_comprobante, cantidad, margen_ganancia, fecha_venta, metodo, banco, contado, id_sucursal) 
		        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
