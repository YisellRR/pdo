...
class compra
{
	...
	public function ListarProducto($id_producto, $desde, $hasta, $sucursal)
	{
		try
		{
			...
			$rango = ($desde=='')? "":"AND fecha_compra >= '$desde' AND fecha_compra <= '$hasta'";
			...
			WHERE v.id_producto = ? $rango $sucursal");
	...
	public function AgrupadoProducto($desde, $hasta)
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT v.fecha_compra, p.producto, SUM(v.cantidad) as cantidad, SUM(v.total) as total, p.precio_costo FROM compras v LEFT JOIN productos p ON v.id_producto = p.id LEFT JOIN clientes c ON v.id_cliente = c.id WHERE CAST(v.fecha_compra AS date) >= ? AND CAST(v.fecha_compra AS date) <= ? AND v.anulado = 0 GROUP BY v.id_producto ORDER BY v.id_compra DESC");
	...
	public function AgrupadoProductoSucursal($desde, $hasta, $id_sucursal)
	{
		try
		{
			
			$stm = $this->pdo->prepare("SELECT v.fecha_compra, p.producto, SUM(v.cantidad) as cantidad, SUM(v.total) as total, p.precio_costo FROM compras v LEFT JOIN productos p ON v.id_producto = p.id LEFT JOIN clientes c ON v.id_cliente = c.id WHERE CAST(v.fecha_compra AS date) >= ? AND CAST(v.fecha_compra AS date) <= ? AND v.anulado = 0 AND v.id_sucursal = ? GROUP BY v.id_producto ORDER BY v.id_compra DESC");
	...
	public function Registrar($data)
	{
		...
		$sql = "INSERT INTO compras (id_compra, id_cliente, id_vendedor, id_producto, id_sucursal, precio_compra, precio_min, precio_may, subtotal, descuento, iva, total, comprobante, nro_comprobante, cantidad, margen_ganancia, fecha_compra, metodo, moneda, cambio, banco, contado, facturable) 
		        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
