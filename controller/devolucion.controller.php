...
class devolucionController{
    ...
    public function finalizarDevolucion(){
        require_once 'view/devolucion/finalizar-modal-devolucion.php';
    }
    ...
    public function Guardar(){
        ...
        foreach($this->devolucion_tmp->Listar() as $v){
            ...
            $devolucion->banco = 0;        
            $devolucion->id_sucursal = $_REQUEST['id_sucursal'];    

            //Registrar venta
            $this->model->Registrar($devolucion);
            //Sumar Stock

            $this->producto->AjustarStock($devolucion);
    ...
    public function Anular(){
        foreach($this->model->Listar($_REQUEST['id']) as $v){
            $devolucion = new devolucion();
            $devolucion->id_producto = $v->id_producto;
            $devolucion->cantidad = $v->cantidad*-1;
            $devolucion->id_sucursal = $v->id_sucursal;
            $this->producto->AnularAjuste($devolucion);
        }
